import logging, sys, time, os, timeit
...
def benchmark(ipv4 = True, ipv6 = False, tcp = True, udp = False, csvOutput = '/tmp'):
    ...
    if ipv4 and tcp:
        ...
        for i in xrange(0, 65535):
            ...
            currTime = time.time()
            computationTime = (currTime-prev_time) * 1000
            approxTotalTime += computationTime
            LOG.info('Computation Time for Operation: %f' , computationTime)
            baconFile.write("%d,%s\n" % (i, round(computationTime, 2)))
        v4_rulesetSize += 65535
        LOG.info('Approximate total time for opening 65535 IPv4 TCP ports: %s', round(approxTotalTime, 2))
        baconFile.close()
    ...
    if ipv4 and udp:
        ...
        for i in xrange(0, 65535):
            ...
            baconFile.write("%d,%s\n" % (i, round(computationTime, 2)))
        v4_rulesetSize += 65535
        LOG.info('Approximate total time for opening 65535 IPv4 UDP ports: %s', round(approxTotalTime, 2))
        baconFile.close()
    ...
    if ipv6 and tcp:
        ...
        for i in xrange(0, 65535):
            ...
            LOG.info('Computation Time for Operation: %f' , computationTime)
            baconFile.write("%d,%s\n" % (i, round(computationTime, 2)))
        v6_rulesetSize += 65535
        LOG.info('Approximate total time for opening 65535 IPv6 TCP ports: %s', round(approxTotalTime, 2))
        baconFile.close()
    ...
    if ipv6 and udp:
        ...
        for i in xrange(0, 65535):
            ...
            baconFile.write("%d,%s\n" % (i, round(computationTime, 2)))
        v6_rulesetSize += 65535
        LOG.info('Approximate total time for opening 65535 IPv6 UDP ports: %s', round(approxTotalTime, 2))
        baconFile.close()
    ...
    if ipv4 and tcp:
        ...
        v4_rulesetSize -= 65535
        LOG.info('Approximate total time for closing 65535 IPv4 TCP ports: %s', round(approxTotalTime, 2))
        baconFile.close()
    ...
    if ipv4 and udp:
        ...
        for i in xrange(0, 65535):
            ...
            baconFile.write("%d,%s\n" % (v4_rulesetSize - i, round(computationTime, 2)))
        v4_rulesetSize -= 65535
        LOG.info('Approximate total time for closing 65535 IPv4 UDP ports: %s', round(approxTotalTime, 2))
        baconFile.close()
    ...
    if ipv6 and tcp:
        ...
        for i in xrange(0, 65535):
            ...
            baconFile.write("%d,%s\n" % (v4_rulesetSize - i, round(computationTime, 2)))
        v6_rulesetSize -= 65535
        LOG.info('Approximate total time for closing 65535 IPv6 TCP ports: %s', round(approxTotalTime, 2))
        baconFile.close()
    ...
    if ipv6 and udp:
        ...
        for i in xrange(0, 65535):
            ...
            baconFile.write("%d,%s\n" % (v4_rulesetSize - i, round(computationTime, 2)))
        v6_rulesetSize -= 65535
        LOG.info('Approximate total time for closing 65535 IPv6 UDP ports: %s', round(approxTotalTime, 2))
        baconFile.close()
        # End Benchmark
